...
try {
  ...
  try {
    $null = Update-DreamSkinCodexIcon -Codex $currentCodex -StateRoot $StateRoot
  } catch {
    Write-Warning "Codex icon refresh failed; skin startup will continue: $($_.Exception.Message)"
  }
  ...
  if (Test-DreamSkinStateFromPreviousBoot -State $previousState) {
    $staleStatePath = Archive-DreamSkinStateFile -Path $StatePath
    Write-Warning "Archived Dream Skin state from the previous Windows boot at $staleStatePath"
    $previousState = $null
  }
