...
try {
  ...
  $currentCodex = $registeredInstalls[0]
  ...
  if (-not $NoShortcuts) {
    ...
    $iconPath = Update-DreamSkinCodexIcon -Codex $currentCodex -StateRoot $StateRoot

    foreach ($folder in @($desktop, $startMenu)) {
      $shortcut = $shell.CreateShortcut((Join-Path $folder 'Codex.lnk'))
      $shortcut.TargetPath = $wscript
      $shortcut.Arguments = "`"$launchScript`" $Port"
      $shortcut.WorkingDirectory = $engine.Root
      $shortcut.IconLocation = "$iconPath,0"
      $shortcut.Description = 'Launch Codex and Miku Skin without a console window'
      $shortcut.Save()
  ...
  if ($NoShortcuts) {
    ...
    Write-Host 'Miku Skin installed. Use the Codex shortcut; it asks before restarting an open Codex window.'
  }
} finally {
