...
function Test-DreamSkinIconBytes {
  param([Parameter(Mandatory = $true)][AllowEmptyCollection()][byte[]]$Bytes)
  return ($Bytes.Length -ge 6 -and
    $Bytes[0] -eq 0 -and $Bytes[1] -eq 0 -and
    $Bytes[2] -eq 1 -and $Bytes[3] -eq 0 -and
    ($Bytes[4] -ne 0 -or $Bytes[5] -ne 0))
}

function Get-DreamSkinCodexIconSource {
  param([Parameter(Mandatory = $true)][object]$Codex)

  $packageRoot = [System.IO.Path]::GetFullPath("$($Codex.PackageRoot)")
  foreach ($relative in @(
    'app\resources\icon-chatgpt.ico',
    'app\resources\app.ico',
    'app\resources\chatgpt-tray-light.ico'
  )) {
    $candidate = [System.IO.Path]::GetFullPath((Join-Path $packageRoot $relative))
    if (-not (Test-DreamSkinPathWithin -Path $candidate -Root $packageRoot) -or
      -not (Test-Path -LiteralPath $candidate -PathType Leaf)) {
      continue
    }
    if (Get-Command Assert-DreamSkinNoReparseComponents -ErrorAction SilentlyContinue) {
      Assert-DreamSkinNoReparseComponents -Path $candidate
    }
    $bytes = [System.IO.File]::ReadAllBytes($candidate)
    if (Test-DreamSkinIconBytes -Bytes $bytes) {
      return [pscustomobject]@{ Path = $candidate; Bytes = $bytes; RelativePath = $relative }
    }
  }
  return $null
}

function Update-DreamSkinCodexIcon {
  param(
  ...
  $iconPath = Join-Path $fullStateRoot 'codex.ico'
  $markerPath = Join-Path $fullStateRoot 'codex-icon.json'
  Assert-DreamSkinNoReparseComponents -Path $iconPath
  Assert-DreamSkinNoReparseComponents -Path $markerPath

  $source = Get-DreamSkinCodexIconSource -Codex $Codex
  $iconBytes = if ($null -ne $source) { $source.Bytes } else { $null }
  $sourceLabel = if ($null -ne $source) { $source.RelativePath } else { 'executable-resource' }
  if ($null -eq $iconBytes) {
    $executable = [System.IO.Path]::GetFullPath("$($Codex.Executable)")
    if (-not (Test-DreamSkinPathWithin -Path $executable -Root "$($Codex.PackageRoot)") -or
      -not (Test-Path -LiteralPath $executable -PathType Leaf)) {
      throw 'The registered Codex executable cannot be used as an icon source.'
    }
    try {
      Add-Type -AssemblyName System.Drawing -ErrorAction Stop
      $icon = [System.Drawing.Icon]::ExtractAssociatedIcon($executable)
      if ($null -eq $icon) { throw 'The executable has no associated icon.' }
      ...
      if (Test-Path -LiteralPath $iconPath -PathType Leaf) {
        $cachedBytes = [System.IO.File]::ReadAllBytes($iconPath)
        if (Test-DreamSkinIconBytes -Bytes $cachedBytes) { return $iconPath }
      }
      throw "The official Codex icon could not be cached: $($_.Exception.Message)"
    }
  }
  if (-not (Test-DreamSkinIconBytes -Bytes $iconBytes)) {
    throw 'The registered Codex package returned an invalid icon.'
  }

  $previousBytes = if (Test-Path -LiteralPath $iconPath -PathType Leaf) {
    [System.IO.File]::ReadAllBytes($iconPath)
  } else { $null }
  if ($null -eq $previousBytes -or -not (Test-DreamSkinBytesEqual -Left $previousBytes -Right $iconBytes)) {
    Write-DreamSkinBytesAtomically -Path $iconPath -Bytes $iconBytes -ExpectedBytes $previousBytes
  }

  $marker = [ordered]@{
    schemaVersion = 1
    packageFullName = "$($Codex.PackageFullName)"
    packageVersion = "$($Codex.Version)"
    source = $sourceLabel
  } | ConvertTo-Json
...
function Install-DreamSkinRuntimeEngine {
  ...
  $required = @(
    ...
    'assets\theme.json',
    'assets\presets\dalao-dianyan\theme.json',
    'assets\presets\dalao-dianyan\hero.webp',
    'assets\presets\deepspace-dawn\theme.json',
    'assets\presets\deepspace-dawn\hero.webp',
    'assets\presets\deepspace-star\theme.json',
    'assets\presets\deepspace-star\hero.webp',
    'assets\presets\genshin-dawn\theme.json',
    'assets\presets\genshin-dawn\hero.webp',
    'assets\presets\genshin-night\theme.json',
    'assets\presets\genshin-night\hero.webp',
    'assets\presets\miku-488137\theme.json',
    'assets\presets\miku-488137\hero.webp',
    'assets\presets\miku-488137\logo.webp',
    'assets\presets\miku-488137\polaroid.webp',
    'assets\presets\naruto-hokage\theme.json',
    'assets\presets\naruto-hokage\hero.webp',
    'assets\presets\naruto-sasuke\theme.json',
    'assets\presets\naruto-sasuke\hero.webp',
    'assets\presets\wuthering-echo\theme.json',
    'assets\presets\wuthering-echo\hero.webp',
    'assets\presets\wuthering-tide\theme.json',
    'assets\presets\wuthering-tide\hero.webp',
    'assets\pets\miku-future\pet.json',
    'assets\pets\miku-future\spritesheet.webp',
    ...
    'scripts\install-miku-future.ps1',
    'scripts\install-miku-future.mjs',
    'scripts\install-pet.mjs',
    'scripts\pet-image-metadata.mjs',
    'scripts\pet-resource-limits.mjs'
  )
...
function Get-DreamSkinSystemBootTimeUtc {
  try {
    $bootTime = (Get-CimInstance Win32_OperatingSystem -ErrorAction Stop).LastBootUpTime
    if ($bootTime -isnot [datetime]) { $bootTime = [datetime]$bootTime }
...
function Test-DreamSkinStateFromPreviousBoot {
  param(
    [AllowNull()][object]$State,
    [AllowNull()][object]$BootTimeUtc = (Get-DreamSkinSystemBootTimeUtc)
  )
  if ($null -eq $State -or -not $State.injectorStartedAt -or $null -eq $BootTimeUtc) { return $false }
  try {
    ...
    return $startedAt -lt ([datetime]$BootTimeUtc).ToUniversalTime().AddSeconds(-2)
  } catch {
    return $false
  }
}
