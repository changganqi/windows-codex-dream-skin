...
import { validateImageMetadata } from "./pet-image-metadata.mjs";
import { parseBoundedJson, readBoundedFile, RESOURCE_LIMITS } from "./pet-resource-limits.mjs";
...
async function syncPath(path) {
  ...
  try {
    ...
    // Windows does not expose fsync for directory handles. File writes are
    // still synced; directory durability is provided by the completed rename.
    if (!(["EPERM", "EINVAL", "EBADF"].includes(error?.code) && process.platform === "win32")) throw error;
  } finally {
    await handle.close();
  }
}
