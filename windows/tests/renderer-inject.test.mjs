...
const buildPayload = (config = {}, branding = {}, catalog = [], currentSource = "built-in", uiPreferences = {}) => template
  .replace("__DREAM_CSS_JSON__", JSON.stringify(".fixture { color: blue; }"))
  .replace("__DREAM_ART_JSON__", JSON.stringify("data:image/png;base64,AA=="))
  .replace("__DREAM_THEME_JSON__", JSON.stringify(config))
  .replace("__DREAM_BRANDING_JSON__", JSON.stringify(branding))
  .replace("__DREAM_CATALOG_JSON__", JSON.stringify(catalog))
  .replace("__DREAM_CURRENT_SOURCE_JSON__", JSON.stringify(currentSource))
...
function createFixture({
  ...
  const createElement = (tagName) => {
    ...
    const element = {
      ...
      appendChild(node) { node.parentElement = this; this.children.push(node); if (node.id) nodes.set(node.id, node); },
      addEventListener(name, listener) { this[`on${name}`] = listener; },
      click() { this.onclick?.({ target: this }); },
      querySelector(selector) {
        const role = /\[data-dream-role="([^"]+)"\]/.exec(selector)?.[1];
        if (!role) return null;
        const queue = [...this.children];
        while (queue.length) {
          const node = queue.shift();
          if (node.dataset?.dreamRole === role) return node;
          queue.push(...(node.children ?? []));
        }
        return null;
      },
      remove() { nodes.delete(this.id); this.parentElement?.children?.splice(this.parentElement.children.indexOf(this), 1); },
    };
    return element;
  };
  if (staleSkin) {
...
assert.equal(nativeResult.installed, true);
assert.equal(native.rootClasses.has("codex-dream-skin"), false);
assert.equal(native.nodes.has("codex-dream-skin-style"), true);
assert.equal(native.nodes.has("codex-dream-theme-center"), true);
assert.equal(native.nodes.has("codex-dream-skin-chrome"), false);
...
trigger.click();
const backdrop = menuRoot.children[1];
...
assert.equal(hiddenPolaroid.nodes.get("codex-dream-polaroid"), undefined,
  "a disabled polaroid preference must keep the card out of the document");
assert.match(css, /\[data-dream-role="theme-trigger"\][\s\S]*-webkit-app-region:\s*no-drag/,
