...
try {
  ...
  foreach ($requiredShortcutBinding in @(
    ...
    '$shortcut.Arguments = "`"$launchScript`" $Port"',
    ...
    "Join-Path `$folder 'Codex.lnk'",
    'Update-DreamSkinCodexIcon -Codex $currentCodex -StateRoot $StateRoot'
  )) {
  ...
  if ([regex]::Matches($installSource, '-ExecutionPolicy RemoteSigned').Count -ne 3 -or
  ...
  if (-not $installSource.Contains("Remove-Item -LiteralPath (Join-Path `$folder 'Codex Dream Skin.lnk')")) {
    throw 'Installer does not retire the old main launch shortcut after creating Codex.lnk.'
  }
  ...
  $bootTime = [datetime]'2026-07-18T12:00:00Z'
  $previousBootState = [pscustomobject]@{ injectorStartedAt = '2026-07-18T11:59:00Z' }
  $currentBootState = [pscustomobject]@{ injectorStartedAt = '2026-07-18T12:01:00Z' }
  if (-not (Test-DreamSkinStateFromPreviousBoot -State $previousBootState -BootTimeUtc $bootTime) -or
    (Test-DreamSkinStateFromPreviousBoot -State $currentBootState -BootTimeUtc $bootTime) -or
    (Test-DreamSkinStateFromPreviousBoot -State ([pscustomobject]@{ injectorStartedAt = 'bad' }) `
      -BootTimeUtc $bootTime)) {
    throw 'Previous-boot state detection is not deterministic or accepts an invalid timestamp.'
  }

  $nodeForStop = Get-DreamSkinNodeRuntime
  $stopProbeScript = Join-Path $temporaryRoot 'stop-probe.mjs'
  [System.IO.File]::WriteAllText(
    $stopProbeScript, 'setInterval(() => {}, 1000);', [System.Text.UTF8Encoding]::new($false, $true)
  )
  $stopBrowserId = [guid]::NewGuid().ToString()
  $stopProbe = Start-Process -FilePath $nodeForStop.Path -ArgumentList @(
    $stopProbeScript, '--watch', '--port', '49341', '--browser-id', $stopBrowserId
  ) -WindowStyle Hidden -PassThru
  try {
    Start-Sleep -Milliseconds 250
    $stopStartedAt = Get-DreamSkinProcessStartedAt -ProcessId $stopProbe.Id
    if (-not $stopStartedAt) { throw 'Stop regression process did not start.' }
    $stopState = [pscustomobject]@{
      injectorPid = $stopProbe.Id
      injectorStartedAt = $stopStartedAt
      injectorPath = $stopProbeScript
      nodePath = $nodeForStop.Path
      port = 49341
      browserId = $stopBrowserId
    }
    if (-not (Stop-DreamSkinRecordedInjector -State $stopState) -or
      (Get-Process -Id $stopProbe.Id -ErrorAction SilentlyContinue)) {
      throw 'Recorded injector stop did not wait for the real process exit under Windows PowerShell 5.1.'
    }
  } finally {
    if (Get-Process -Id $stopProbe.Id -ErrorAction SilentlyContinue) {
      Stop-Process -Id $stopProbe.Id -Force -ErrorAction SilentlyContinue
    }
  }
  Remove-Item -LiteralPath $stopProbeScript -Force
  ...
  $iconPackageRoot = Join-Path $temporaryRoot 'icon-package'
  $iconResourceRoot = Join-Path $iconPackageRoot 'app\resources'
  $iconExecutable = Join-Path $iconPackageRoot 'app\ChatGPT.exe'
  $iconStateRoot = Join-Path $temporaryRoot 'icon-state'
  New-Item -ItemType Directory -Path $iconResourceRoot -Force | Out-Null
  [System.IO.File]::WriteAllBytes($iconExecutable, [byte[]](0x4d, 0x5a))
  $iconSource = Join-Path $iconResourceRoot 'icon-chatgpt.ico'
  $firstIconBytes = [byte[]](0, 0, 1, 0, 1, 0, 11, 22, 33, 44)
  [System.IO.File]::WriteAllBytes($iconSource, $firstIconBytes)
  $fakeIconCodex = [pscustomobject]@{
    PackageRoot = $iconPackageRoot
    PackageFullName = 'OpenAI.Codex_1.0.0.0_x64__test'
    Version = '1.0.0.0'
    Executable = $iconExecutable
  }
  $stableIcon = Update-DreamSkinCodexIcon -Codex $fakeIconCodex -StateRoot $iconStateRoot
  if (-not (Test-DreamSkinBytesEqual -Left $firstIconBytes -Right ([System.IO.File]::ReadAllBytes($stableIcon)))) {
    throw 'Installer did not cache the registered Codex package icon at a stable path.'
  }
  $secondIconBytes = [byte[]](0, 0, 1, 0, 1, 0, 55, 66, 77, 88)
  [System.IO.File]::WriteAllBytes($iconSource, $secondIconBytes)
  $fakeIconCodex.PackageFullName = 'OpenAI.Codex_2.0.0.0_x64__test'
  $fakeIconCodex.Version = '2.0.0.0'
  $stableIcon = Update-DreamSkinCodexIcon -Codex $fakeIconCodex -StateRoot $iconStateRoot
  $iconMarker = (Read-DreamSkinUtf8File -Path (Join-Path $iconStateRoot 'codex-icon.json')) | ConvertFrom-Json
  if (-not (Test-DreamSkinBytesEqual -Left $secondIconBytes -Right ([System.IO.File]::ReadAllBytes($stableIcon))) -or
    $iconMarker.packageFullName -cne $fakeIconCodex.PackageFullName) {
    throw 'Stable Codex icon cache did not refresh after a Store package version change.'
  }
  ...
  if (-not $startSource.Contains('Update-DreamSkinCodexIcon -Codex $currentCodex -StateRoot $StateRoot') -or
    -not $startSource.Contains('skin startup will continue')) {
    throw 'Start does not refresh the stable official icon after a Store package update, or icon failure blocks startup.'
  }
  if (-not $startSource.Contains('Test-DreamSkinStateFromPreviousBoot -State $previousState') -or
  ...
  $hiddenLauncher = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\launch-dream-skin.vbs')
  $hiddenLauncherPowerShell = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\launch-dream-skin.ps1')
  foreach ($hiddenLauncherContract in @(
